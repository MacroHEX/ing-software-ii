// src/app/api/reporte/organizadores/route.ts
  ...
  try {
      ...
      <html>
        ...
        <body class="bg-gray-100 flex justify-center items-start min-h-screen p-5">
          <div class="container w-full max-w-3xl p-5 bg-white shadow-lg rounded-lg flex flex-col justify-between">
            <h1 class="text-3xl font-bold text-center mb-4">Reporte de Organizadores - Sistema de Invitaciones</h1>
            <h3 class="text-xl font-semibold text-center mb-6">Módulo: Gestión de Organizadores</h3>
      ...
      .map(
        (organizador) => `
                  <tr class="border-t hover:bg-gray-100">
                    <td class="px-4 py-2">${organizador.organizadorid}</td>
                    <td class="px-4 py-2">${organizador.usuario.nombreusuario}</td>
                    <td class="px-4 py-2">${organizador.evento.nombre}</td>
                  </tr>`
      )
      .join('')}
              </tbody>
    ...
    return new NextResponse(pdfBuffer, {
      ...
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="reporte_organizadores.pdf"',
      },
    });
  } catch (error) {
