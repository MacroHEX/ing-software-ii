// src/app/api/reporte/tipoeventos/route.ts
  ...
  try {
      ...
      <html>
        ...
        <body class="bg-gray-100 flex justify-center items-start min-h-screen p-5">
          <div class="container w-full max-w-3xl p-5 bg-white shadow-lg rounded-lg flex flex-col justify-between">
            <h1 class="text-3xl font-bold text-center mb-4">Reporte de Tipos de Evento - Sistema de Invitaciones</h1>
            <h3 class="text-xl font-semibold text-center mb-6">Módulo: Gestión de Tipos de Evento</h3>
      ...
      .map(
        (tipoEvento) => `
                  <tr class="border-t hover:bg-gray-100">
                    <td class="px-4 py-2">${tipoEvento.tipoeventoid}</td>
                    <td class="px-4 py-2">${tipoEvento.descripcion}</td>
                  </tr>`
      )
      .join('')}
              </tbody>
    ...
    return new NextResponse(pdfBuffer, {
      ...
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="reporte_tipo_eventos.pdf"',
      },
    });
  } catch (error) {
